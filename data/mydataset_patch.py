...
if __name__ == '__main__':
    ...
    train_data = SR3_Dataset_finetune_patch(np.array(target_paths)[train_index],land_01_path,mask_path,np.array(data_paths)[train_index],'sp',patch_size=128)
    ...
    max_t = torch.from_numpy(np.load("/home/data/downscaling/downscaling_1023/data/train_dataset/max_new_10.npy", mmap_mode='r+')).float()
    min_t = torch.from_numpy(np.load("/home/data/downscaling/downscaling_1023/data/train_dataset/min_new_10.npy", mmap_mode='r+')).float()
    figure,ax=plt.subplots(1,2,figsize=(5,10))
    ax[0].imshow(max_t[2],vmin=-10,vmax=10,cmap="RdBu_r")
    ax[1].imshow(min_t[2],vmin=-10,vmax=10,cmap="RdBu_r")
    ...
    for i,ret in enumerate(train_loader):
        ...
        if i % 10==0:
            ...
            print(ret["HR"][:,0].max(),ret["INTERPOLATED"][:,0].min())
    ...
    #     # print(lr_data.shape,hr_target.shape,mask_data.shape,land_01_data.shape,physical.shape)
            ...
            ax[0,0].imshow(ret["INTERPOLATED"][0,0],vmin=-2,vmax=2,cmap="RdBu_r")
        # ax[0,1].imshow(ret["INTERPOLATED"][0,1],vmin=0,vmax=0.5)
        # ax[0,1].imshow(ret["INTERPOLATED"][0,2],vmin=0,vmax=0.5)
            ax[0,1].imshow(ret["HR"][0,0],vmin=-2,vmax=2,cmap="RdBu_r")
            ax[1,0].imshow(ret["INTERPOLATED"][0,0],vmin=0,vmax=1,cmap="RdBu_r")
        # ax[0,1].imshow(ret["INTERPOLATED"][0,1],vmin=0,vmax=0.5)
        # ax[0,1].imshow(ret["INTERPOLATED"][0,2],vmin=0,vmax=0.5)
            ax[1,1].imshow(ret["HR"][0,0],vmin=0,vmax=1,cmap="RdBu_r")
        # ax[1,1].imshow(ret["Control_data"]['850hpa'][0,1],vmin=-5,vmax=5)
        # ax[1,0].imshow(ret["Control_data"]['850hpa'][0,2],vmin=-5,vmax=5)
